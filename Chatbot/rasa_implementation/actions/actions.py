...
df = pd.read_csv("dataset/goodreads_chatbot_cleaned.csv")
...
class ActionSearchBooks(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # Applicazione filtri uno per uno
        try:
            # Filtro per genere
            if genre:
...
class ActionShowSelectedBook(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # Se non c'è, lo estrae dal messaggio
        if not book_index:
            import re
        ...
        try:
            # Conversione dell'indice a numero (1-based → 0-based)
